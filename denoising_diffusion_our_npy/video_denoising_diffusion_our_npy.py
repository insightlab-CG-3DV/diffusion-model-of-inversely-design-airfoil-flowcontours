...
class GaussianDiffusion(nn.Module):
    ...
    #@torch.inference_mode()
    def p_sample(self, x, t, cond = None, clip_denoised = True, guidance_scale = 1.,is_infer=False):
        with torch.no_grad():
            b, *_, device = *x.shape, x.device
            model_mean, _, model_log_variance = self.p_mean_variance(x = x, t = t, clip_denoised = clip_denoised, cond = cond, guidance_scale = guidance_scale)
            noise = torch.randn_like(x)
        ...
        if((t<100) and is_infer):
            ...
            u=u.unsqueeze(0)
            v=v.unsqueeze(0)
            pde_loss=get_ns_bounded_loss(u,v)
            L_pde = torch.norm(pde_loss, 2)/(96*96)
            grad_x_cur_pde = torch.autograd.grad(outputs=L_pde, inputs=x_cur,retain_graph=True, allow_unused=True)[0]
            return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise-1.0*grad_x_cur_pde
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    
    # @torch.inference_mode()
    def p_sample_loop(self, shape, cond = None, guidance_scale = 1.,is_infer=False):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), cond = cond, guidance_scale = guidance_scale,is_infer=is_infer)

        return unnormalize_img(img)
        # return img

    # @torch.inference_mode()
    def sample(self, cond = None, batch_size = 16, guidance_scale = 1.,is_infer=False):
        batch_size = cond.shape[0] if exists(cond) else batch_size
        image_size = self.image_size
        channels = self.channels
        num_frames = self.num_frames
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, num_frames, image_size, image_size), cond = cond, guidance_scale = guidance_scale,is_infer=is_infer)
    ...
    def eval_test(
        ...
        if test_cond_full_repeated is not None:
            ...
            for cond in batched_test_cond:
                ...
                samples_list=[]
                for i in range(2):
                    samples = ema_model.sample(cond=cond, guidance_scale = guidance_scale,is_infer=True)
                    samples_list.append(samples.unsqueeze(0))
                samples=torch.cat(samples_list,dim=0)
                samples=samples.mean(dim=0)
                #samples = ema_model.sample(cond=cond, guidance_scale = guidance_scale)
