...
def unnorm_eval(arr, min_val, max_val):
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.cond_attention_tokens = cond_attention_tokens if not per_frame_cond else 10
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t,mask,is_mask=False, cond = None, noise = None, **kwargs):
        ...
        if is_mask:
            if self.loss_type == 'l1':
                loss = F.l1_loss(mask*noise, mask*x_recon)
                # torchvision.utils.save_image((mask*noise)[0,0,5,:,:],"./noise.png")
                # torchvision.utils.save_image((mask*x_recon)[0,0,5,:,:],"./x_recon.png")
                # print(aaaaa)
            elif self.loss_type == 'l2':
                loss = F.mse_loss(mask*noise, mask*x_recon)
                # torchvision.utils.save_image((mask*noise)[0,0,5,:,:],"./noise.png")
                # torchvision.utils.save_image((mask*x_recon)[0,0,5,:,:],"./x_recon.png")
                # print(aaaaa)
            else:
    ...
    def forward(self, x,mask, *args, **kwargs):
        ...
        # print("x",x)
        # print("x_shape",x.shape)
        # torchvision.utils.save_image(x[0,0,0,:,:],"./x.png")
        # torchvision.utils.save_image(masked_x[0,0,0,:,:],"./x_mask.png")
        #print(aaaaaaa)
        return self.p_losses(x, t,mask,False, *args, **kwargs)#+self.p_losses(x, t, mask,True,*args, **kwargs)
...
class Dataset(data.Dataset):
    def __init__(
        ...
        # print(self.paths_pressure)
        # print(aaaaaa)
        #assert all([int(p.stem) == i for i, p in enumerate(self.paths_pressure)]), 'file position is not equal to index'                
        ...
        #     self.max_pressure = torch.max(self.frame_ranges[:,0])


        #     # save min/max values for transforming 
        #     data = [
        #         ['max_pressure', self.max_pressure.item()],
        #     ]
        ...
        label_file = folder + 'Token.csv'

        labels_np = np.genfromtxt(label_file, delimiter=',')
        # print("label_shape",labels_np.shape)
        # print("per_frame_cond",per_frame_cond)
        # print(aaaaaaaa)
        if per_frame_cond:
            # strain = 0.2
        ...
        # print("normalized_label",self.labels)
        # print(aaaaa)
    ...
    def __getitem__(self, index):
        ...
        if self.reference_frame == 'fluid':
            ...
            # mask=(mask[:,:,0].unsqueeze(0).unsqueeze(1)).repeat(1,10,1,1)
            # print(tensor.shape)
            # print(mask.shape)
            # masked_tensor=tensor*mask
            # torchvision.utils.save_image(tensor[:,6,:,:],"./tensor.png")
            # torchvision.utils.save_image(masked_tensor[:,6,:,:],"./masked.png")
            # print(tensor.min())
            # print(tensor.max())
            # print(aaaaaa)
            #print("before_normalize",tensor)
            # tensor = torch.where(tensor>10.0,10.0,tensor)
            # tensor = torch.where(tensor<-10.0,-10.0,tensor)
            # tensor= self.normalize(tensor, tensor.min(), tensor.max())
            #print("after_normalize",tensor)
            # os.makedirs("./masked_gt",exist_ok=True)
            # torchvision.utils.save_image(masked_tensor[:,5,:,:],"./masked_gt/"+str(index)+".png")
        ...
        elif self.reference_frame == 'eulerian':
        ...
        # print("tensor2",self.cast_num_frames_fn(tensor).shape)
        return self.cast_num_frames_fn(tensor), labels
        #,self.cast_num_frames_fn(masked_tensor),self.cast_num_frames_fn(mask)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.test_batch_size = test_batch_size  # since evaluation requires more memory
        ...
        self.ds_test = Dataset(folder, image_size, labels_scaling=self.ds.labels_scaling, \
    ...
    def load(
        ...
        with open(path, 'rb') as f:
            ...
            #print( loaded_obj )
            keys_list = list(loaded_obj['model'].keys())
            for key in keys_list:
                #print(key)
                if 'module.' in key:
                    deal_key = key.replace('module.', '')
                    loaded_obj['model'][deal_key] = loaded_obj['model'][key]
                    del loaded_obj['model'][key]
            keys_list = list(loaded_obj['ema'].keys())
            for key in keys_list:
                #print(key)
                if 'module.' in key:
                    deal_key = key.replace('module.', '')
                    loaded_obj['ema'][deal_key] = loaded_obj['ema'][key]
                    del loaded_obj['ema'][key]    

        try:
            self.model.load_state_dict(loaded_obj['model'], strict = True)
        except RuntimeError:
        ...
        #try:
        self.ema_model.load_state_dict(loaded_obj['ema'], strict = True)
        #except RuntimeError:
    ...
    def train(
        ...
        progress_bar = tqdm(range(self.train_num_steps), disable=not self.accelerator.is_local_main_process)
        while self.step <= self.train_num_steps:
            ...
            #     print("train time",time.time()-start_time)
                ...
                self.opt.step()
            # self.accprint("training loss",loss.item())
            self.log_fn({'training loss': loss.item()}, step = self.step)
            # progress_bar.set_postfix({"Loss": f"loss.item()"})
            # print(aaaaaa)
            # print(f"Process {self.accelerator.state.local_process_index}: Waiting for everyone to reach this point.")
            # self.accelerator.wait_for_everyone()
            # print(aaaaaa)
            # if self.step % 8== 0 and self.step>8:
            #     self.accelerator.wait_for_everyone()
